  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        ...
        <Link href="/admin/products/new" className="rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white">
          + Novo produto
        </Link>
      </div>
      ...
      <input
        ...
        className="mb-4 w-full max-w-sm rounded-lg border border-border px-3 py-2 text-sm"
      />
            ...
            {items?.map((p) => (
              <tr key={p.id} className="border-b border-border last:border-0">
                <td className="flex items-center gap-3 px-4 py-3">
                  ...
                  <Link href={`/admin/products/${p.id}`} className="font-medium text-foreground hover:text-brand">
                    {p.name}
                  </Link>
                </td>
                <td className="px-4 py-3 text-foreground/70">{STATUS_LABEL[p.status] ?? p.status}</td>
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-2">
                    <button
                      ...
                      className="rounded px-2 py-1 text-xs text-foreground/60 hover:bg-black/5 disabled:opacity-50"
                    >
                      Duplicar
                    </button>
                    {confirmDeleteId === p.id ? (
                      <>
                        <button
                          ...
                          className="rounded px-2 py-1 text-xs font-medium text-price hover:bg-price/10"
                        >
                          Confirmar exclusão
                        </button>
                        <button
                          onClick={() => setConfirmDeleteId(null)}
                          className="rounded px-2 py-1 text-xs text-foreground/60 hover:bg-black/5"
                        >
                          Cancelar
                        </button>
                      </>

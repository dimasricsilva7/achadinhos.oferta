...
export function StickyBuyBar({
  ...
  return (
    // Truly fixed to the viewport (not `sticky`, which only pins within its own
    // scroll container and — depending on the surrounding layout's height — can end
    // up just sitting at the end of the page content instead of tracking scroll).
    // `left-0 right-0` + the `mx-auto max-w-lg` wrapper below keeps it centered and
    // capped at the same width as the page content on wide viewports.
    <div className="safe-bottom fixed inset-x-0 bottom-0 z-30 bg-surface/95 backdrop-blur">
